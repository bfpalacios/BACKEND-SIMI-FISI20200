...
public class HorarioRepositoryImpl implements HorarioRepository {
	...
	public Horario crearHorario(Horario horario) {
		String insertQuery = "INSERT INTO tmhorario_grupo_horario (FK_ID_GRUPOHORARIO, FK_ID_DIA, FK_ID_HORA) "
				+ "VALUES (?, ?, ?)";
		int success = this.jdbcTemplate.update(insertQuery, horario.getIdGrupoHorario(),
				horario.getIdDia(), horario.getIdHora());
	...
	public Horario editHorario(Horario horario, int id) {
		String query = "UPDATE tmhorario_grupo_horario SET FK_ID_GRUPOHORARIO = ?, FK_ID_DIA = ?, FK_ID_HORA = ? "
				+ "WHERE CHORARIO = "+ id;
	...
	public boolean deleteHorario(int chorario) {
		String query = "DELETE FROM tmhorario_grupo_horario WHERE FK_ID_GRUPOHORARIO = ?";
	...
	public List<HorarioDTO> getHorarios() {
		String query = "SELECT * FROM tmhorario_grupo_horario AS ho "
				+ "INNER JOIN tmgrupo_horario AS gh ON gh.ID_GRUPOHORARIO = ho.FK_ID_GRUPOHORARIO "
				+ "INNER JOIN txdias_clase AS di ON di.ID_DIA = ho.FK_ID_DIA "
	...
	public List<HorarioDTO> getHorariosById(int id) {
		String query = "SELECT * FROM tmhorario_grupo_horario AS ho "
				+ "INNER JOIN tmgrupo_horario AS gh ON gh.ID_GRUPOHORARIO = ho.FK_ID_GRUPOHORARIO "
				+ "INNER JOIN txdias_clase AS di ON di.ID_DIA = ho.FK_ID_DIA "
				+ "INNER JOIN txhoras_clase AS hr ON hr.ID_HORA = ho.FK_ID_HORA WHERE FK_ID_GRUPOHORARIO = " + id;
